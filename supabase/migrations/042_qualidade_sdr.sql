-- View `gold.qualidade_sdr`: avalia a atuação do SDR em cada etapa de cadência
-- (C1..C5) com base em 25 critérios qualitativos do Kommo (custom fields).
--
-- Cada critério tem 4 valores possíveis no Kommo:
--   Bom           → 1.0
--   Parcial       → 0.5
--   Não           → 0.0
--   Não se aplica → NULL (peso é normalizado entre os outros critérios da etapa)
--
-- Pesos dos critérios (somam 100 dentro de cada etapa):
--   C1: 30/25/20/15/10
--   C2: 30/25/15/15/15
--   C3: 25/25/20/15/15
--   C4: 25/25/20/15/15
...
--
-- Fórmula:
--   nota_etapa = SUM(valor × peso) / SUM(peso onde valor IS NOT NULL) × 100
--   nota_geral = 0.20·C1 + 0.20·C2 + 0.15·C3 + 0.20·C4 + 0.25·C5
--
-- Granularidade: 1 linha por lead fechado. SDR vem do custom field 'SDR' do
-- lead. Período de análise = data_fechamento_fmt.

CREATE OR REPLACE FUNCTION gold.qualidade_score(v text)
RETURNS numeric
LANGUAGE sql IMMUTABLE
AS $$
  SELECT CASE
    WHEN v = 'Bom'           THEN 1.0
    WHEN v = 'Parcial'       THEN 0.5
    WHEN v = 'Não'           THEN 0.0
    WHEN v = 'Não se aplica' THEN NULL
    ELSE NULL
  END::numeric
$$;

CREATE OR REPLACE FUNCTION gold.qualidade_nota_etapa(
  v1 text, p1 numeric,
  v2 text, p2 numeric,
  v3 text, p3 numeric,
  v4 text, p4 numeric,
...
AS $$
  SELECT CASE
    WHEN SUM(peso) FILTER (WHERE score IS NOT NULL) > 0
      THEN ROUND(SUM(score * peso) FILTER (WHERE score IS NOT NULL) / SUM(peso) FILTER (WHERE score IS NOT NULL) * 100, 1)
    ELSE NULL
  END
  FROM (VALUES
    (gold.qualidade_score(v1), p1),
    (gold.qualidade_score(v2), p2),
    (gold.qualidade_score(v3), p3),
    (gold.qualidade_score(v4), p4),
    (gold.qualidade_score(v5), p5)
  ) AS t(score, peso)
$$;
...
WITH base AS (
  SELECT
    lc.lead_id, lc.lead_name, lc.data_fechamento_fmt, lc.canal_entrada, lc.vendedor,
  ...
  WHERE NOT lc.cancelado
)
SELECT
  b.lead_id, b.lead_name, b.data_fechamento_fmt, b.canal_entrada, b.vendedor, b.sdr,
